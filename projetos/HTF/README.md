...
A introdução de multi-encoders transformers (METFs) no discriminador do Social GAN, junto à variação extensiva de parâmetros e estruturas, resultou no treinamento de diversos modelos em busca da melhoria da arquitetura de baseline. A melhoria quantitativa obtida no desenvolvimento do modelo profundo foi da ordem de 10%, considerando um cenário de treinamento reduzido, com treinamento em torno de 5 mil interações. Por um lado, os ganhos quantitativos são satisfatórios para o desenvolvimento efetuado. Por outro lado, a avaliação por meio das métricas quantitativas utilizadas, ADE e FDE, não reflete as eventuais melhorias realizadas no modelo, uma vez que não está sendo avaliado a quantidade de colisões em interações cena a cena. Além disso, devido à condição otimizada e especialista do modelo de baseline, ganhos quantitativos, embora existentes, limitaram-se a melhorias de 10%. É importante ressaltar que o objetivo deste trabalho se tratava do desenvolvimento de duas arquiteturas de modelos profundos na Social GAN, contemplando o desenvolvimento do módulo de interação espacial (SIM) e o discriminador com base em células transformers em configuração encoder-only, conforme o artigo de referência [1]. O desenvolvimento do discriminador da Social GAN com base em múltiplos encoders transformers foi satisfatoriamente realizado e responsável por efetuar melhorias na arquitetura de baseline. Por outro lado, a inexistência de um loader capaz de embaralhar dados com base em cenas – gerando batches a partir do embaralhamento de trajetórias observadas e ground truth de indivíduos –, na arquitetura de baseline, inviabilizou o redesenvolvimento tanto do loader quanto o desenvolvimento do SIM, uma vez que a inserção dessas alterações iriam modificar a base de funcionamento do algoritmo de AI/ML e comprometeria a base de comparação entre os modelos, dado que o modelo atualizado avaliaria cenas e interações sociais de pedestres, enquanto o baseline avaliaria somente trajetórias descritas puramente pela posição. Considera-se o desenvolvimento das melhorias satisfatórias para o desenvolvimento do projeto de Social GANs, efetuando-se o ciclo completo de desenvolvimento, depuração, testes e otimização.
