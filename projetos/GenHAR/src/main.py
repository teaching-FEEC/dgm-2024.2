...
def main(config_path):
    ...
    for dataset_conf in config["datasets"]:
        ...
        for transformation_config in config["transformations"]:
            ...
            log.print_debug(
                f"{dataset_conf['name']}{x_t_train.shape} transform info-> name:{transformation_config['name']}, shape:{x_t_train.shape} "
            )
            ...
            for generative_model_config in config["generative_models"]:
                ...
                data_generate = DataGenerate(
                    generative_model_config, dataset_conf["name"], transformation_config["name"]
                )
                ...
                evaluator = Evaluator(
                    config_eval, df_trans_train_, df_trans_test_, df_trans_val_, synthetic_df
                )
                evaluator.eval()
