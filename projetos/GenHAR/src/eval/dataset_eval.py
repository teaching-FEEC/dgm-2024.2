...
class TimeSeriesDatasetEvaluator:
    ...
    def plot_i_samples(self, activity_names=['sit', 'stand', 'walk', 'stair up', 'stair down', 'run'], n_samples=3, reshape=False):
        ...
        label_map = {name: i  for i, name in enumerate(activity_names)}  # Assuming IDs are 1-indexed
        ...
        # Plot
        fig, axs = plt.subplots(n_activities, n_samples, figsize=(12, n_activities * 3), squeeze=False)
    ...
    def plot_random_sensor_samples_single_dataset(self,df, activity_names, num_samples=3,sensor="accel"):
        """
        ...
        fig, axes = plt.subplots(ncols=num_samples, nrows=6, figsize=(5* num_samples, 5*6 ), sharex=True, sharey=True)
        ...
        for act_id, activity_name in enumerate(activity_names):
            ...
            if len(activity_data) < num_samples:
                print(f"Not enough samples available for activity: {activity_name}. Requested: {num_samples}, Available: {len(activity_data)}")
                continue
