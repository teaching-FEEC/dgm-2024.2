...
def TimeGAN(data, parameters):
    ...
    Embedder = Time_GAN_module(
        input_size=z_dim, output_size=hidden_dim, hidden_dim=hidden_dim, n_layers=num_layers
    )
    Recovery = Time_GAN_module(
        input_size=hidden_dim, output_size=dim, hidden_dim=hidden_dim, n_layers=num_layers
    )
    Generator = Time_GAN_module(
        input_size=dim, output_size=hidden_dim, hidden_dim=hidden_dim, n_layers=num_layers
    )
    Supervisor = Time_GAN_module(
        input_size=hidden_dim, output_size=hidden_dim, hidden_dim=hidden_dim, n_layers=num_layers
    )
    Discriminator = Time_GAN_module(
        ...
        for batch_index, X in enumerate(loader):
            ...
            if e in range(1, epoch) and batch_index == 0:
                print(
                    ...
                    + str(epoch)
                    + ", s_loss: "
                    + str(np.sqrt(G_loss_S.detach().numpy()))
                )
    ...
    for itt in range(epoch):
        ...
        for kk in range(2):
            ...
            G_loss_Ue = binary_cross_entropy_loss(Y_fake_e, torch.ones_like(Y_fake_e))
        ...
        print(
            ...
            + str(epoch)
            + ", D_loss: "
            + str(D_loss.detach().numpy())
            + ", G_loss: "
            + str(G_loss.detach().numpy())
            + ", E_loss_t0: "
            + str(np.sqrt(E_loss0.detach().numpy()))
        )
