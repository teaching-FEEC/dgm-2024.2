...
class REMIFullSongTransformerDataset(Dataset):
  ...
  def build_dataset(self):
    ...
    for i, p in enumerate(self.pieces):
      try:
        ...
        print(f"Arquivo {p} carregado com sucesso.")
      except Exception as e:
        print(f"Erro ao carregar o arquivo {p}: {e}")
        continue  # Ignore o arquivo problemático
  ...
  def __getitem__(self, idx):
    ...
    print(f"Carregando o item {idx}...")  # Adicione um print para identificar qual item está causando o problema
    try:
        bar_events, st_bar, bar_pos, enc_n_bars = self.get_sample_from_file(idx)
        print(f"Item {idx} carregado com sucesso.")
    except Exception as e:
        print(f"Erro ao carregar o item {idx}: {e}")
        raise e  # Levante o erro novamente para ver qual é o problema.
...
if __name__ == "__main__":
  ...
  dloader = DataLoader(dset, batch_size=4, shuffle=False, num_workers=0)
