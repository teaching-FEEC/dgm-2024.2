...
from torch import nn, optim, tensor, long
import torch
import numpy as np
import pickle
...
# Função de ajuste dos dados para o formato de entrada do modelo
def adjust_data_for_training(bar_pos, events):
    enc_input = []
    dec_input = []
    dec_target = []
    
    # Preenchendo enc_input, dec_input e dec_target com base nos eventos
    for i in range(len(events) - 1):
        if events[i]['name'] == 'Note_Pitch':
            enc_input.append(events[i]['value'])  # Note_Pitch
            dec_input.append(events[i+1]['value'])  # Note_Duration
            dec_target.append(events[i+1]['value'])  # Note_Duration (alvo para a decodificação)

    # Convertendo para tensores para o modelo
    enc_input = tensor(enc_input, dtype=long)
    dec_input = tensor(dec_input, dtype=long)
    dec_target = tensor(dec_target, dtype=long)
...
def train_model(epoch, model, dloader, dloader_val, optim, sched, loss_history):
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    # Substitua esta parte pela aplicação da função adjust_data_for_training
    #batch_enc_inp = batch_samples['enc_input']
    #batch_dec_inp = batch_samples['dec_input']
    #batch_dec_tgt = batch_samples['dec_target']

    # Ajustando os dados com a função que criamos
    #batch_enc_inp, batch_dec_inp, batch_dec_tgt = adjust_data_for_training(batch_samples['bar_pos'], batch_samples['events'])

    # Impressão de diagnóstico das dimensões
    print(f'Batch {batch_idx}:')
    #print(f'  batch_enc_inp shape: {batch_enc_inp.shape}')
    #print(f'  batch_dec_inp shape: {batch_dec_inp.shape}')
    #print(f'  batch_dec_tgt shape: {batch_dec_tgt.shape}')

    batch_enc_inp = batch_samples['enc_input'].permute(2, 0, 1).to(device)
    print('deu bom!')
    batch_dec_inp = batch_samples['dec_input'].permute(1, 0).to(device)
    batch_dec_tgt = batch_samples['dec_target'].permute(1, 0).to(device).long()
    ...
    # Adicionar perdas da época ao histórico
    loss_history['recons_loss'].append(recons_loss_ema)
    loss_history['kl_loss'].append(kl_loss_ema)

    # Salvar histórico de perda por época
    loss_path = os.path.join(ckpt_dir, f'epoch_{epoch:03d}_loss.pkl')
    with open(loss_path, 'wb') as f:
        pickle.dump({'recons_loss': recons_loss_ema, 'kl_loss': kl_loss_ema}, f)

    # Salvar pesos do modelo ao final da época
    weights_path = os.path.join(params_dir, f'epoch_{epoch:03d}-RC_{recons_loss_ema:.3f}-KL_{kl_loss_ema:.3f}-model.pt')
...
if __name__ == "__main__":
  ...
  loss_history = {'recons_loss': [], 'kl_loss': []}
  ...
  dset.pieces = dset.pieces[:10]
  dloader = DataLoader(dset, batch_size=config['data']['batch_size'], shuffle=True, num_workers=0)
  dloader_val = DataLoader(dset_val, batch_size=config['data']['batch_size'], shuffle=True, num_workers=0)
  ...
  for ep in range(config['training']['max_epochs']):
    train_model(ep+1, model, dloader, dloader_val, optimizer, scheduler, loss_history)