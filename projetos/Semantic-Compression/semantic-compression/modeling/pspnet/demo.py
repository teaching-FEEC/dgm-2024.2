...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes_pspnet101.yaml', help='config file')
    parser.add_argument('--image', type=str, default='dataset_aumentado/', help='input image')
    parser.add_argument('--result', type=str, default='dataset_aumentado/', help='result image')
...
def main():
    ...
    #colors = np.loadtxt('../semseg/data/cityscapes/cityscapes_colors.txt').astype('uint8')
...
def test(model, image_path, classes, mean, std, base_size, crop_h, crop_w, scales, colors):
    for nome_arquivo in os.listdir(image_path):
    	caminho_completo = os.path.join(image_path, nome_arquivo)
    
    
    
    
    	image = cv2.imread(caminho_completo, cv2.IMREAD_COLOR)  # BGR 3 channel ndarray wiht shape H * W * 3
    	...
    	gray_path = os.path.join(args.result, nome_arquivo + '_gray.png')
    	color_path = os.path.join(args.result, nome_arquivo + '_color.png')
