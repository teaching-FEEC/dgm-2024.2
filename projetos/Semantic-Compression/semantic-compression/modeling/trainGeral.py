...
def fine_tune(model, dataloader_train, dataloader_val, dataloader_labels_train, dataloader_labels_val, epochs, discriminate_every, save_every, model_fname):
    gan_losses_train = []
    l2_losses_train = []
    d_losses_train = []
    gan_losses_val = []
    ...
    for epoch in pbar:
        ...
        for batch, batch_labels in zip(dataloader_train, dataloader_labels_train):
            i+=1
            b_gan, b_l2, b_d, b_loss = model.train_step(batch, batch_labels, discriminating=(i % discriminate_every == 0))
            gan_train += b_gan
            l2_train += b_l2
            d_train += b_d
            loss_train += b_loss
        ...
        d_val = 0.
        loss_val = 0.
        for batch, batch_labels in zip(dataloader_val, dataloader_labels_val):
            b_gan, b_l2, b_d, b_loss = model.test_step(batch, batch_labels)
            gan_val += b_gan
            l2_val += b_l2
            d_val += b_d
            loss_val += b_loss
...
def main(path_train, path_val, path_labels_train, path_labels_val, H, W, filters, n_blocks, channels, momentum, batch_size, ae_lr, gan_lr, epochs_pt, epochs_ft, discriminate_every, save_every, SC):
    ...
    train_set = image_dataset_from_directory(
        path_train, labels=None, batch_size=batch_size, image_size=(H, W), shuffle=False
    )
    val_set = image_dataset_from_directory(
        path_val, labels=None, batch_size=batch_size, image_size=(H, W), shuffle=False
    )

    train_labels_set = image_dataset_from_directory(
        path_labels_train, labels=None, batch_size=batch_size, image_size=(H, W), shuffle=False
    )
    val_labels_set = image_dataset_from_directory(
        path_labels_val, labels=None, batch_size=batch_size, image_size=(H, W), shuffle=False
    )
    ...
    x_labels_train = []
    for batch in train_labels_set:
        x_labels_train.append(batch)
        x_labels_train = tf.concat(x_labels_train, axis=0)

    x_labels_val = []
    for batch in val_labels_set:
        x_labels_val.append(batch)
        x_labels_val = tf.concat(x_labels_val, axis=0)

    ae = AutoEncoder(filters, n_blocks, channels, n_convs=4, sigma=1000., levels=5, l_min=-2, l_max=2, momentum=momentum, SC = 0)
    ...
    gan = GCGAN(
        filters, n_blocks, channels, n_convs=4, sigma=1000, levels=5, l_min=-2, l_max=2, lambda_d=10, momentum=momentum, SC = SC
    )
    gan.compile(
    ...
    gan.autoencoder.load_weights(f'../models/ae_{model_fname}.weights.h5')

    gan, gan_losses_train, l2_losses_train, gan_losses_val, l2_losses_val = fine_tune(gan, train_set, val_set, train_labels_set, val_labels_set, epochs_ft, discriminate_every, save_every, model_fname)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_labels_train', type=str, default='../data/train', help='Caminho para o dataset de labels de treino')
    parser.add_argument('--path_labels_val', type=str, default='../data/test', help='Caminho para o dataset labels de validação')
    ...
    parser.add_argument('--SC', type=int, default=0, help='Opção para qual gan usar')
    ...
    path_labels_val = args.path_labels_val
    H = args.H
    W = args.W
    width = args.width
    depth = args.depth
    ...
    main(path_train, path_val, path_labels_train, path_labels_val, H, W, width, depth, batch_size, d_lr, g_lr, epochs, save_every, SC)